...
namespace ccfrag{
	...
	public:
		...
		public:
			...
			template<typename I>
			static uint32_t execute(I begin, I end, uint32_t c = 0)
			{
		...
		public:
			...
			bool read(T& data, size_t count = sizeof(T))
			{
				...
				size_t shift = 0;
				while(count){
					data |= static_cast<T>((*begin)&0xFF) << shift;
					shift += 8;
					--count;
					advance(1);
				}
		...
		static bool decode(std::vector<char>& output, const std::vector<char>& input)
		{
			...
			if(!in.read(ID1) || ID1 != 0x1F){
				fprintf(stderr, "ID1 error %02x\n", ID1);
				return false;
			}
			uint8_t ID2;
			if(!in.read(ID2) || ID2 != 0x8B){
				fprintf(stderr, "ID2 error %02x\n", ID2);
				return false;
			}
			uint8_t CM;
			if(!in.read(CM) || CM != CM_DEFLATE){
				fprintf(stderr, "CM error %02x\n", CM);
				return false;
			}
			uint8_t FLG;
			if(!in.read(FLG) || FLG & FLG_RESERVED){
				fprintf(stderr, "FLG error %02x\n", FLG);
				return false;
			}
			uint32_t MTIME; // Modification TIME
			if(!in.read(MTIME)){
				fprintf(stderr, "MTIME error %08x\n", MTIME);
				return false;
			}
			uint8_t XFL; // eXtra FLags
			if(!in.read(XFL)){
				fprintf(stderr, "XFL error %02x\n", XFL);
				return false;
			}
			uint8_t OS;
			if(!in.read(OS)){
				fprintf(stderr, "OS error %02x\n", OS);
				return false;
			}
			if(FLG & FLG_FEXTRA){
				uint16_t XLEN;
				if(!in.read(XLEN)){
					fprintf(stderr, "XLEN error %02x\n", XLEN);
					return false;
				}
				in.advance(XLEN);
			}
			...
			if(FLG & FLG_FHCRC){
				...
				uint32_t c = crc32::execute(input.data(), input.data() + input.size() - in.size()) & 0xFFFF;
				if(c != CRC16){
					return false;
				}
			}
			if(in.size() < 8){
				fprintf(stderr, "too short size %zd\n", in.size());
				return false;
			}
			size_t header = input.size() - in.size();
			size_t footer = 8;
			if(!ccfrag::deflate::decode(output, {in.begin, in.end - 8})){
				fprintf(stderr, "deflate::decode error\n");
				return false;
			}
			in.advance(in.size() - 8);
			uint32_t CRC32;
			uint32_t c = crc32::execute(output.data(), output.data() + output.size());
			if(!in.read(CRC32, 4) || c != CRC32){
				fprintf(stderr, "data size %zd\n", output.size());
				fprintf(stderr, "CRC32 error %08x != %08x\n", c, CRC32);
				//return false;
			}
			uint32_t ISIZE;
			uint32_t isize = (output.size() & 0xFFFFFFFF);
			if(!in.read(ISIZE, 4) || isize != ISIZE){
				fprintf(stderr, "ISIZE error %8x != %8x\n", isize, ISIZE);
				return false;
			}
			return true;
		}
